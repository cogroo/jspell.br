...
:: ENGLISH ::
This dictionary is based on the Ispell version of the Portuguese dictionary created by José João de Almeida (jj@di.uminho.pt) and Ulisses Pinto, and thus is covered by his original Free Software Foundation (FSF)  GPL version 2 license.

All modifications previous to 25/1/2005 to the affix file and wordlist to make it work with MySpell are copyright Artur Correia (artur.correia@netvisao.pt).

All modifications on and after 25/1/2005 to the affix file and word-list to make it work with MySpell are copyright Rui Vilela (ruivilela@di.uminho.pt) from Linguateca (http://www.linguateca.pt). The dictionary is also maintained by Alberto Simões (ambs@di.uminho.pt) and José João de Almeida (jj@di.uminho.pt) of Natura project(http://natura.di.uminho.pt).

New dictionary versions may be available at:
http://linguateca.di.uminho.pt/
or
http://natura.di.uminho.pt/
...
1. Utilizámos a nova versão do ispell para português como base do trabalho (o ficheiro affix e o dicionário são novos)
2. Utilizou-se um script de perl para a transformação do ficheiro affix para MySpell. Supostamente agora temos menos bugs.
3. Graças à preciosa colaboração de Daniel Andrade (mop12079@mail.telepac.pt), que encontrou - e resolveu - este bug, agora as palavras sugeridas incluem também palavras com caracteres portugueses (çãàâ, etc...)
